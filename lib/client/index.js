...
client.init = function init (callback) {
  ...
  eruda && eruda.position && eruda.position({x:10,y:10})
...
client.load = function load (serverSettings, callback) {
  ...
  client.dataExtent = function dataExtent () {
    if (client.entries.length > 0) {
      ...
      return [new Date(client.now - times.hours(HOURS_IN_BAR).msecs), new Date(client.now)];
    }
  };
  ...
  function brushed () {
    ...
    function updateCurrentSGV (entry) {
      ...
      console.log(new Date(),"BG:",JSON.stringify(value));
    ...
    function clearCurrentSGV () {
      console.log(new Date(), "clear current bg");
      currentBG.text('---');
  ...
  function prepareEntries () {
    ...
    var tooOld = client.now - times.hours(HOURS_IN_BAR).msecs;
